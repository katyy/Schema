namespace Schema.Core.SqlQueries
{
    ...
    public class MsSqlQueries : ISqlQueries
    {
        ...
        public string SelectViewTriggers
        {
            ...
            {
                return
                    @"SELECT t.name table_name as " +
                          @", triger.triger_name as " +
                          @", triger.triger_event as " +
                          @", triger.type as " +
                          @", triger.type_desc as" + 
                     @" FROM 
                         (SELECT te.type_desc triger_event,tr.name triger_name, tr.parent_id,tr.type,tr.type_desc 
        ...
        public string SelectViewIndexes
        {
            ...
            {
                return
                    @"SELECT index_table.table_name as " +
                         @", c.name column_name as " +
                         @", index_table.name as " +
                         @", index_table.type_desc as " +
                         @", index_table.is_unique as " +
                         @", index_table.is_descending_key as " + 
                    @" FROM
                     (SELECT t.name table_name,indexes.* 
                      FROM 
                         (SELECT i.object_id,i.name,i.type_desc,i.is_unique,ic.index_column_id,ic.column_id,ic.is_descending_key
        ...
        public string SelectFunction
        {
            ...
            {
                return
                    @"SELECT fun.function_name as " + 
                         @", fun.parametr_name as " +
                         @", fun.type as " +
                         @", fun.type_desc as " +
                         @", ty.name as " + 
                         @", fun.max_length as " +
